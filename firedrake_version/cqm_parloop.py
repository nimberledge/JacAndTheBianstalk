...
def computeSkewness(mesh, P0=None):
    ...
    double dpm11 = sqrt (pow((x1 - mx1), 2) +  pow((y1 - my1), 2) );
    double dpm22 = sqrt (pow((x2 - mx2), 2) +  pow((y2 - my2), 2) );
    double dpm33 = sqrt (pow((x3 - mx3), 2) +  pow((y3 - my3), 2) );
    ...
    double pi = 3.1415926535897;
    double min_t = 0.0;
    double t1 = acos ((lnx1 * lox1 + lny1 * loy1) / (dpm11 * dm23));
    double t2 = pi - t1;
    min_t = fmin(t1, t2);

    double t3 = acos ((lnx2 * lox2 + lny2 * loy2) / (dpm22 * dm13));
    min_t = fmin(min_t, t3);
    double t4 = pi - t3;
    min_t = fmin(min_t, t4);

    double t5 = acos ((lnx3 * lox3 + lny3 * loy3) / (dpm33 * dm12));
    min_t = fmin(min_t, t5);
    double t6 = pi - t5;
    min_t = fmin(min_t, t6);
